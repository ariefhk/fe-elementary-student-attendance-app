...
export default function AdminAddStudentDialog({ open = false, onOpenChange, onClose }) {
  const [createStudent, { isLoading: isLoadingCreateStudent }] = useCreateStudentMutation()
      ...
      <AlertDialogContent className="px-0 max-w-[600px] font-poppins">
        ...
        <AlertDialogHeader className=" max-h-[400px] px-8 flex-col gap-y-0 items-center gap-x-16    ">
          <AlertDialogTitle className="space-y-5  flex flex-col items-center w-full">
            <span className="text-fs24_36 font-semibold  text-color-1">Input Data Siswa</span>
            <Separator />
          </AlertDialogTitle>
          <Form {...form}>
            <form
        ...
        <AlertDialogFooter className=" px-8">
          ...
          <Button
            ...
            {isLoadingCreateStudent && <BsArrowRepeat className="animate-spin  w-5 h-5 flex-shrink-0" />}{" "}
            Tambah
          </Button>
        </AlertDialogFooter>
      </AlertDialogContent>
